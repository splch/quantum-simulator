...
simulator and by a numpy reference, with every amplitude compared to 1e-5.
    ...
    uv run tests/ref.py                  # 20 circuits of 40 gates on 5 qubits, JS lane
    uv run tests/ref.py --native         # also build and run the native binary
    uv run tests/ref.py --seed 7 --keep  # keep tests/diff_tmp.bend for a look

Qubit 0 is the most significant bit on both sides. The generated program is
written beside this script as diff_tmp.bend and imports the sources from
../src, since a Bend import is relative to the importing file. Angles and
raw-unitary entries are rounded to F32 before both sides see them, so the only
differences left are the simulator's F32 arithmetic against numpy's float64.
"""
import argparse
import math
import re
...
def c_lit(z):
    return f"S.C{{{lit(z.real)}, {lit(z.imag)}}}"
...
def one_qubit_table(rng):
    ...
    return {
        "h": ("G.h", "S.H()", np.array([[RH, RH], [RH, -RH]], complex)),
        "x": ("G.x", "S.X()", np.array([[0, 1], [1, 0]], complex)),
        "y": ("G.y", "S.Y()", np.array([[0, -1j], [1j, 0]])),
        "z": ("G.z", "S.Z()", np.diag([1, -1]).astype(complex)),
        "s": ("G.s", "S.S()", np.diag([1, 1j])),
        "sdg": ("G.sdg", "S.Sdg()", np.diag([1, -1j])),
        "t": ("G.t", "S.T()", np.diag([1, np.exp(1j * math.pi / 4)])),
        "tdg": ("G.tdg", "S.Tdg()", np.diag([1, np.exp(-1j * math.pi / 4)])),
        "p": (f"G.p({lit(phi)}, ", f"S.P({lit(phi)})", np.diag([1, np.exp(1j * phi)])),
        "rx": (f"G.rx({lit(th)}, ", f"S.RX({lit(th)})", np.array([[c, -1j * s], [-1j * s, c]])),
        "ry": (f"G.ry({lit(th)}, ", f"S.RY({lit(th)})", np.array([[c, -s], [s, c]], complex)),
        "rz": (f"G.rz({lit(th)}, ", f"S.RZ({lit(th)})", np.diag([np.exp(-1j * th / 2), np.exp(1j * th / 2)])),
    }
...
def random_gate(rng, n):
    ...
    *cs, q = (int(v) for v in rng.choice(n, k + 1, replace=False))
    if kind == "ctrl_named":
        ...
        gate = f"S.U({c_lit(m[0, 0])}, {c_lit(m[0, 1])}, {c_lit(m[1, 0])}, {c_lit(m[1, 1])})"
